...
def t_net(model_Pnet16,model_Rnet24,model_Onet36):    
    ...
    basedir = '../widerface-test/dataset'
    filename = './train_103918_gt.txt'  # 獲取偵測框的ground truth值 # 改
 ...
 #   assert len(det_boxes) == num_of_images # len(det_boxes) == num_of_images应该为TRUE，如果不是，则返回错误。"incorrect detections or ground truths"
    ...
    #for i in tqdm(range(5)):
        ...
        draw_cv2_bboxes_dets(img, dets, color='g')
        draw_cv2_bboxes_gts(img, gts, color='r')
        cv2.imshow('img',img)
        cv2.waitKey(0)
...
def draw_cv2_bboxes_gts(img, bboxes, color):
  if(color == 'g'):
    c = (0,255,0)
  if(color == 'r'):
    c = (0,0,255)
  for bbox in bboxes:
    ...
    #cv2.putText(img, str(np.round(score, 2)), (bbox[0], bbox[1]), cv2.FONT_HERSHEY_TRIPLEX, 1, color=c)
    #w = (int(bbox[2]/2)-bbox[0])*2 
    #h = (int(bbox[3]/2)-bbox[1])*2 
    cv2.rectangle(img, (bbox[0], bbox[1]), (bbox[0]+(int(bbox[2]/2)-bbox[0])*2 ,bbox[1]+(int(bbox[3]/2)-bbox[1])*2 ), c, thickness=2)

def draw_cv2_bboxes_dets(img, bboxes,color): 
  for bbox in bboxes:
    score = bbox[4]  
    bbox = list(map(int, bbox[0:4]))
...
if __name__ == '__main__':
    ...
    model_Pnet16 = model.net16_2p_Depthwise()
    ...
    Pnet16_ckpt_path = ("ckpt\\"+"Pnet16_800_epochs_1915573net16_2p_Depthwise"+"\\cp-{epoch:04d}.ckpt".format(epoch = 195))
